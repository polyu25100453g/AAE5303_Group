...
# Usage: from repo root, ./scripts/docker_run_amtown02.sh
# Requires: bag at /mnt/d/AMtown02.bag (WSL) or D:\AMtown02.bag (Windows path in WSL = /mnt/d/...)
...
BAG_HOST="${BAG_PATH:-/mnt/d/AMtown02.bag}"
BAG_IN_CONTAINER="/data/AMtown02.bag"

IMAGE="${DOCKER_IMAGE:-liangyu99/orbslam3_ros1:latest}"
CONTAINER_NAME="${CONTAINER_NAME:-orbslam3_amtown02}"

if [ ! -f "$BAG_HOST" ]; then
  echo "Bag not found at: $BAG_HOST"
  echo "Set BAG_PATH if needed, e.g. BAG_PATH=/path/to/AMtown02.bag $0"
  exit 1
fi
